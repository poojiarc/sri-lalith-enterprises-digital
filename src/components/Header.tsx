  ...
  <div className="hidden sm:block">
    ...
    <p className="text-xs text-muted-foreground">
      Packaging & Tape Solutions
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navigation.map((item) => (
              ...
              >
                ...
                {item.children && openDropdown === item.name && (
                  <div className="absolute top-full left-0 mt-2 w-56 bg-brand-black/95 backdrop-blur-lg rounded-xl shadow-xl border border-white/10">
                    {item.children.map((child) => (
                      <Link
                        key={child.name}
                        to={child.href}
          ...
          <Link
            to="/contact"
            className="hidden lg:inline-flex px-6 py-2.5 bg-green-gradient text-white font-semibold rounded-xl shadow-glow-green hover:scale-105 transition"
          >
            Contact Us
          </Link>
